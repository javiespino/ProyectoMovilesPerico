...
import com.adorno.mappers.UserDetails2USerEntity;
...
public class UserService {
	...
	public UserService(UserRepository userRepository, RoleRepository roleRepository,
			...
			UserEntity2UserInfoDTOMapper userEntity2UserInfoDTOMapper, PasswordEncoder passwordEncoder) {
		super();
	...
	private void crearDios() {
		UserDetails god = User.withUsername("Dios").password(passwordEncoder.encode("1234")).roles("ADMIN").build();
		if(userRepository.findByUsername(god.getUsername()).isEmpty())
			userRepository.save(new UserDetails2USerEntity().map(god));
